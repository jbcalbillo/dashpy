...
# Título de la vista
st.title("Distribucion de Candidatos por Fuente de Reclutamiento")
...
if uploaded_file_json is not None:
    ...
    if isinstance(data, list):
        ...
        # Agrupar por Fuente de Reclutamiento
        grouped = df.groupby('fuenteReclutamiento').size().reset_index(name='cantidad')
        
        # Mostrar tabla
        st.subheader("Tabla de Candidatos por Fuente de Reclutamiento")
        st.table(grouped)
        
        # Crear un gráfico de barras usando Altair
        chart = alt.Chart(grouped).mark_bar(size=20).encode(
            x=alt.X('fuenteReclutamiento:O', sort='-y', axis=alt.Axis(labelAngle=-45, title='Fuente de Reclutamiento')),
            y=alt.Y('cantidad:Q', axis=alt.Axis(title='Cantidad')),
            color=alt.Color('fuenteReclutamiento:N', legend=None, scale=alt.Scale(scheme='tableau20')),
            tooltip=['fuenteReclutamiento', 'cantidad']
        ).properties(
            width=800,
            height=500
        )
        
        # Agregar una capa de texto para mostrar las cantidades al costado de cada barra
        ...
        final_chart = (chart + text).configure_title(
            ...
            title='Distribucion de Candidatos por Fuente de Reclutamiento'
        )
